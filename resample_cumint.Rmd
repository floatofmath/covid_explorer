...
The algorithm to connect events with past cases is fairly simple. In essence we start with the earliest outcome (death/recovery) and randomly connect it to a case confirmed in the past (relative to the event). We then progress to the next later event, until finally all events (including censored) are connected to one starting date.

We then estimate the CFR using Fine and Gray's model for competing risk data to estimate the CFR.

I now generate 20 samples of time to event datasets for each country and use that to estimate cumulative incidence curves. Currently, sampling is such that each event is matched a case in the past with equal probablity. This can be improved by applying sampling weights from a survival distributions. However, that makes the process quite a bit slower and results do not differ substantially, so for the moment I use uniform sampling weights.
...
data_long %>% resample_surv('China',B=20) -> cumint_china 
...
cumint_austria <-  data_long %>% resample_surv('Austria',B=50) 
...
While Austria and Germany appear quite similar to South Korea. Italy and Sweden show quite different results. Italy has a substantially higher mortality rate than then the other countries in this comparison. About `r cumint_italy %>% average_rsurv %>% predict_cuminc %$% {1-normCFR}`% recoverd and `r cumint_italy %>% average_rsurv %>% predict_cuminc %$% {normCFR}`% died. *This may be due to specifics of the testing practice with only hospitalized subjects tested (based on hearsay need source)*. Sweden which also follows a different strategy, also appears to have a high mortality rate and at least for now a lower recovery rate. About `r cumint_sweden %>% average_rsurv %>% predict_cuminc %$% {1-normCFR}`% recoverd and `r cumint_sweden %>% average_rsurv %>% predict_cuminc %$% {normCFR}`% died. Clearly there has to be some explanation in the way Sweden tests cases and determines recovery. 

For Austria, I estimate about `r cumint_austria %>% average_rsurv %>% predict_cuminc %$% {1-normCFR}`% recoverd and `r cumint_austria %>% average_rsurv %>% predict_cuminc %$% {normCFR}`% died. For Germany we obtain an estimate about `r cumint_germany %>% average_rsurv %>% predict_cuminc %$% {1-normCFR}`% recoverd and `r cumint_germany %>% average_rsurv %>% predict_cuminc %$% {normCFR}`% died. In Germany the cumulative incidence curve for recovery is substantially longer than for mortality. This is probably because Germany had a few very early cases, and can be seen also in the long horizontal segment in the recovery curve between about 30 and 50 days.
