...
csse_confirmed <- read_csv('data/CSSE_COVID-19/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv')#read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv"))#~/Projekte/Science Stuff/covid/data/time_series_19-covid-Confirmed.csv")
csse_deaths <- read_csv('data/CSSE_COVID-19/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv')#read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_deaths_global.csv"))#~/Projekte/Science Stuff/covid/data/time_series_19-covid-Deaths.csv")
csse_recovered <- read_csv('data/CSSE_COVID-19/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.csv')#read_csv(url("https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_recovered_global.csv"))#~/Projekte/Science Stuff/covid/data/time_series_19-covid-Recovered.csv")
# population <- read_csv(url('https://raw.githubusercontent.com/datasets/population/master/data/population.csv'))
# current_population <- population %>% group_by(`Country Name`) %>% arrange(desc(Year)) %>% slice(1:1)

```

## Pre-processing

I grab the data mainly from the Coronavirus COVID-19 Global Cases by Johns Hopkins CSSE Dashboard which provides raw-data at https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/. Data are updated daily, but may not be as recent as depicted on the online dashboard.

I perform a little bit of pre-processing. Mainly I collapse data for the US and China. These data are provided on the provincial level, which makes it difficult to visualise using mapping software. Having these data on the country level let's me easily use different mapping tools that provide mapping data on the country level. 
...
Finally, a big caveat is that we can only see figures for subjects that were tested positive. Different testing capacities and approaches therefore can have a large influence on the numbers. 
...
## Synnchronized curves


```{r core plots 2}
data_long %>% filterCore() %>% synchronize_confirmed('Italy',cut=10) %>% 
  group_by(`Country/Region`) %>% 
  {
  ggplot(aes(Date,Confirmed),data=.) + 
  geom_path(aes(col=`Country/Region`)) +
  geom_text(aes(Date,Confirmed,label=`Country/Region`,hjust=0),data=filterLatest(.)) + 
...
data_long %>% filter(`Country/Region` %in% c('Italy','Austria','Sweden','United Kingdom','Germany','Switzerland')) %>% 
  filterRecent() %>% filter(Deaths > 10) %>% synchronize_deaths('Italy',cut=20) %>% arrange(Date) %>% 
  {
  ggplot(aes(Date,Deaths,group=`Country/Region`),data=.) + #geom_path(aes(col=`Country/Region`)) +
      geom_smooth(aes(col=`Country/Region`,fill=`Country/Region`),span=10,alpha=.1) +
  geom_text(aes(Date,Deaths,label=`Country/Region`,hjust=0.5,vjust = 0),data=filterLatest(.))  + 
      scale_y_log10(label=scales::comma) + 
      theme_minimal() 
    }
...
data_long %>% filter(`Country/Region` %in% c('Italy','Austria','Sweden','United Kingdom','Germany','Switzerland')) %>% 
  filterRecent() %>% filter(Confirmed > 50) %>% synchronize_deaths('Italy',cut=20) %>% 
  {
  ggplot(aes(Date,ADTD,group=`Country/Region`),data=.) + #geom_path(aes(col=`Country/Region`)) +
      geom_smooth(aes(col=`Country/Region`,fill=`Country/Region`),span=10,alpha=.1) +
  geom_text(aes(Date,ADTD,label=`Country/Region`,hjust=0.5,vjust = 0),data=filterLatest(.)) + theme_minimal() 
